#!/usr/bin/env python3
"""Controls the LEDs on the ReSpeaker 2mic HAT with enhanced processing."""
import argparse
import asyncio
import logging
import time
...
_BLUE = (0, 0, 255)
_GREEN = (0, 255, 0)
_PURPLE = (255, 0, 255)  # Added for processing state
...
class LEDsEventHandler(AsyncEventHandler):
    ...
    async def query_perplexity(self, text):
        ...
        try:
            ...
            _LOGGER.debug(f"Querying Perplexity with: {text}")
            ...
            _LOGGER.debug(f"Perplexity response: {answer}")
            return answer
    ...
    async def handle_event(self, event: Event) -> bool:
        ...
        if StreamingStarted.is_type(event.type):
            ...
            # Wake word detected - start enhanced processing
            self.color(_BLUE)
            await asyncio.sleep(1.0)
        elif VoiceStarted.is_type(event.type):
            self.color(_YELLOW)
        elif Transcript.is_type(event.type):
            # Process transcription with Perplexity
            self.color(_PURPLE)  # Show processing state
            transcript_text = event.data["text"]
            _LOGGER.debug(f"Processing transcript: {transcript_text}")
            
            response = await self.query_perplexity(transcript_text)
            if response:
                _LOGGER.info(f"Perplexity response: {response}")
                # Here we'll add the call to Eleven Labs
